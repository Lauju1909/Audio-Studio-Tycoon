...
def fix_double_money(f):
    ...
    original = c
    # Fix track_income
    c = re.sub(r'self(\.state)?\.money \+= ([^\n]+)\n(\s+)self(\.state)?\.track_income', r'\3self\4.track_income', c)
    # Fix track_expense
    c = re.sub(r'self(\.state)?\.money -= ([^\n]+)\n(\s+)self(\.state)?\.track_expense', r'\3self\4.track_expense', c)
...
for root, dirs, files in os.walk('.'):
    for file in files:
        if file.endswith('.py') and file not in ['fix_money.py', 'gamestate.py', 'snippets.py', 'harden_models.py']:
            fix_double_money(os.path.join(root, file))
